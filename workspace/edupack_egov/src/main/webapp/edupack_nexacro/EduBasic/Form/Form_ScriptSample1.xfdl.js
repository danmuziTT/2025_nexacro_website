(function()
{
    return function()
    {
        ...
        { //onload 시 동기통신 쓰지말것
            ...
            var sParam    = "";
            var sCallBack = "fn_callback";
        	this.transaction(sSvcID, sURL, sInDs, sOutDs, sParam, sCallBack, false); //통신 끝난 트렌젝션을 콜백함수로 보내고 그 뒤 함수가 실행됨
        	//false >> 동기통신을 하겠다 (전송과 응답의 순번이 일정한 통신을 하겠다)
        ...
        {
        	// Select Employees Data 로우가 바뀌면 이에 따라 데이터가 바뀌는 것
            var sDeptCode = this.ds_dept.getColumn(this.ds_dept.rowposition, "CODE");
            var sSvcID    = "svcSelectEmp";
            var sURL      = "EduUrl::select_emp.jsp";
            var sInDs     = "";
