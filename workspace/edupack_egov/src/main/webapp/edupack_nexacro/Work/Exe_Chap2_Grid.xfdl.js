(function()
{
    return function()
    {
        ...
        this.on_create = function()
        {
            ...
            obj.set_positionstep("2");
            obj.set_autofittype("col");
            obj._setContents("<Formats><Format id=\"default\"><Columns><Column size=\"251\"/><Column size=\"159\"/><Column size=\"80\"/><Column size=\"131\"/><Column size=\"200\"/></Columns><Rows><Row size=\"32\" band=\"head\"/><Row size=\"32\"/></Rows><Band id=\"head\"><Cell text=\"ORG_NAME\"/><Cell col=\"1\" text=\"POSITION\"/><Cell col=\"2\" text=\"GENDER\"/><Cell col=\"3\" text=\"PHONE_NO\"/><Cell col=\"4\" text=\"E_MAIL\"/></Band><Band id=\"body\"><Cell text=\"bind:ORG_NAME\" edittype=\"normal\"/><Cell col=\"1\" text=\"bind:POSITION\"/><Cell col=\"2\" text=\"bind:GENDER\"/><Cell col=\"3\" text=\"bind:PHONE_NO\"/><Cell col=\"4\" text=\"bind:E_MAIL\"/></Band></Format></Formats>");
            this.addChild(obj.name, obj);
        ...
        {
           var nColCnt = this.Grid1.getFormatColCount();
           ...
           trace("colcnt: " + nColCnt + ", colcnt1_1: " + nColCnt1_1
                 + "cellcnt: " + nCellCnt + ", cellcnt1_1: " + nCellCnt1_1
           );
        };

        this.Grid_oncellclick = function(obj,e)
        {
           trace("cell: " + e.cell + ", column: " + e.col);
        };

        this.btn_Exe1_3_onclick = function(obj,e)
        {
           var sText = this.Grid1.getCellProperty("body", 1, "text"); // body 안 1번째 cell의 text값 가져오기
           var arrCol = sText.split(":");
           var sColId = arrCol[1];
           trace("Text: " + sText + " , Column: " + sColId);
           // bind:FULL_NAME:ABC
           // arrCol[0] = bind, arrCol[1] = FULL_NAME, arrCol[2] = ABC

           var nCellCnt = this.Grid1.getCellCount("body");
           for (var i = 0; i < nCellCnt; i++) {
              var arrCol = this.Grid1.getCellProperty("body", i, "text").split(":");
              trace(arrCol[1]);
           }
        };
        ...
        {
           /*
           var arrColId = this.Grid1.getCellProperty("body", e.cell, "text").split(":");
           this.Dataset1,set_keystring("S: " + arrColId[1]);
           */
           this.fn_sort(obj, e);
        };

        this.fn_sort = function(obj, e) {
           var arrColId = obj.getCellProperty("body", e.cell, "text").split(":");
           var objDs = obj.getBindDataset();
           objDs.set_keystring("S: " + arrColId[1]);
        }
        this.btn_Exe2_1_onclick = function(obj,e)
        {
           // column 설정
           this.Dataset2.addColumn("COL_CHK", "STRING", 1); // size default: 256
           // row 설정
           var nRowCnt = this.Dataset2.rowcount;
           this.Dataset2.set_enableevent(false); // 이벤트 잠시 멈춤
           for (var i = 0; i < nRowCnt; i++) {
              this.Dataset2.setColumn(i, "COL_CHK", "0");
           }
           this.Dataset2.set_enableevent(true);
           // Grid의 body 맨 앞에 column 추가
           this.Grid2.insertContentsCol("body", 0);
           this.Grid2.setCellProperty("body", 0, "text", "bind:COL_CHK"); // dataset과 binding 되어야 함
        };
        ...
        this.Grid2_onheadclick = function(obj,e)
        {
           // cell이 1번째 일 때
           if (e.cell == 0) {
              var sText = this.Grid2.getCellText(-1, 0); // nRow -1 : head -2 : summary
              sText = (sText == "1" ? "0" : "1");
              // Head
              this.Grid2.setCellProperty("head", 0, "text", sText); // 속성 값 부여

              // Body - dataset binding
              var nRowCnt = this.Dataset2.rowcount;
              this.Dataset2.set_enableevent(false);
              for (var i = 0; i < nRowCnt; i++) {
                 this.Dataset2.setColumn(i, "COL_CHK", sText);
              }
              this.Dataset2.set_enableevent(true);
           }
        };
        ...
        {
           this.Grid3.set_cellmovingtype("col"); // column 끼리 이동 가능
           this.Grid3.set_cellsizingtype("col"); // column 사이즈 변경 가능
        };

        this.btn_Exe3_2_onclick = function(obj,e)
        {
           // column fix
           this.Grid3.setFormatColProperty(2, "band", "left"); // 3번째 속성까지 left로 band 고정
           // row fix
        ...
        {
           // column clear
           this.Grid3.setFormatColProperty(0, "band", "body"); // 3번째 속성까지 body로 고정 해제
           // row clear
        ...
        {
           this.Grid3.setFormatColProperty(2, "size", 100); // name 속성 사이즈 100 설정
           this.Grid3.setFormatColProperty(5, "size", 0); // salary 속성 지우기
        };

        this.btn_Exe3_5_onclick = function(obj,e)
        {
           this.Grid3.setFormatColProperty(5, "size", 80); // 사이즈를 설정하여 salary 속성 나타내기
        };

        this.btn_Exe3_6_onclick = function(obj,e)
        {
           // 개인화
           // true: 디자인 시 설정한 포맷 반환 false (default): 동적으로 변경된 포맷
           var sOrgFormat = this.Grid3.getCurFormatString(true);
           this.Grid3_1.set_formats("<Formats>" + sOrgFormat + "</Formats>");

           var sCurFormat = this.Grid3.getCurFormatString(); // default: false
           this.Grid3_2.set_formats("<Formats>" + sCurFormat + "</Formats>");

        };

        this.Grid4_oncellclick = function(obj,e)
        {
           var nGridRow = this.Grid4.getTreeRow(e.row); // 클릭된 트리 행
           var nStatus = this.Grid4.getTreeStatus(nGridRow); // 현재 트리 상태 -: 1, +: 0 end: 3

           if (nStatus == 3) return;
           nStatus = (nStatus == 1 ? 0 : 1);
        ...
        {
           // keystring
           this.Dataset5.set_keystring("G-CORP,DEPT") // -: 내림차순
        };

        this.btn_Exe5_2_onclick = function(obj,e)
        {
           // 버튼을 누를 때마다 summary 영역이 추가됨
           this.Grid5.appendContentsRow("summary");

           // 5번째 cell의 text 영역에 this.Dataset5.getSum("SALARY")를 추가
           this.Grid5.setCellProperty("summary", 4, "text", this.Dataset5.getSum("SALARY"));

           // 5번째 cell의 displaytype을 number로 설정
           this.Grid5.setCellProperty("summary", 4, "displaytype", "number");
        };

        this.btn_Exe5_3_onclick = function(obj,e)
        {
           this.Grid5.set_summarytype("top"); // summary band를 위로 올림 (expression)
           this.Dataset5.set_reversesubsum(true); // keystring으로 준 dataset의 합계를 위로 올림
        };
        ...
        {
           this.Grid6.set_selecttype(e.postvalue);
        };
        ...
        this.Button00_onclick = function(obj,e)
        {
           ...
           for (var i = 0; i < 10000; i++) {
              var nRow = this.Dataset6.addRow();
              this.Dataset6.copyRow(nRow, this.Dataset6, 0);
              this.Dataset6.setColumn(nRow, "FULL_NAME", "name" + i);
           }
           this.Dataset6.set_enableevent(true);
        };


        this.rdo00_onitemchanged = function(obj,e)
        {
           this.Grid6.set_fastvscrolltype(e.postvalue);
        };
