...
public void saveEdtOption(List<Map<String, Object>> saveList) {
    ...
    for(int i = 0; i <nSize; i++) {
        ...
        int rowType = (int) rowData.get("DataSetRowType");
        
        if(rowType == DataSet.ROW_TYPE_INSERTED) { // insert SQL
            nexaEduMapper.saveInsertOption(rowData);
        } 
        else if(rowType == DataSet.ROW_TYPE_DELETED) { // delete SQL
            nexaEduMapper.saveDeleteOption(rowData);
        }
        else if(rowType == DataSet.ROW_TYPE_UPDATED) { // update SQL
            Map orgData = (Map) rowData.get("DataSetSavedData");
...
public void SaveCancleData(List<Map<String, Object>> saveList) {
    ...
    for (int i = 0; i < nSize; i++) {
        ...
        String existingStatus = nexaEduMapper.getExistingStatus(rowData); // 기존 STATUS 값 조회
...
@Override
public String idFound(List<Map<String, Object>> saveList) {
    Map<String, Object> rowData = saveList.get(0);
    ...
    if (result == 0) {
        ...
        return nexaEduMapper.idFoundResult(rowData); // USER_ID 반환
    }
}

@Override
public String pswFound(List<Map<String, Object>> saveList) {
Map<String, Object> rowData = saveList.get(0);
    
    int result = nexaEduMapper.pswFound(rowData);

    if (result == 0) {
        return "0"; // 0 반환
    } else {
        return nexaEduMapper.pswFoundResult(rowData); 
    }
}

@Override
public void saveProData(List<Map<String, Object>> saveList) {
int nSize = saveList.size();
	
	for(int i =0; i <nSize; i++) {
		Map<String, Object> rowData = saveList.get(i);
		if((int)rowData.get("DataSetRowType")== DataSet.ROW_TYPE_DELETED) { //delete SQL
			nexaEduMapper.deleteProDe(rowData);
			nexaEduMapper.deleteProOp(rowData);
			nexaEduMapper.deletePro(rowData);
		}
	}
}
